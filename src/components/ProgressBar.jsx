...
const ProgressBar = () => {
  ...
  useEffect(() => {
    ...
    if (progressRef.current) {
      ...
      progressRef.current.setAttribute('aria-valuenow', Math.round(initialProgress));
    }

    // Subscribe directly to the store to bypass React renders for high-frequency updates
    const unsubscribe = useStore.subscribe(
      (state) => {
        if (progressRef.current) {
          progressRef.current.style.width = `${state.progress}%`;
          progressRef.current.setAttribute('aria-valuenow', Math.round(state.progress));
        }
      }
    );
