import React, { useEffect, useRef } from 'react';
import { useStore } from '../store/useStore';

/**
 * ProgressBar Component
 *
 * Performance Optimization (Bolt):
 * Bypasses React rendering for high-frequency progress updates by subscribing
 * directly to the Zustand store and manipulating the DOM node via a ref.
 * This avoids expensive reconciliations during batch processing.
 */
const ProgressBar = () => {
  const processing = useStore((state) => state.processing);
  const progressRef = useRef(null);

  useEffect(() => {
    if (!processing || !progressRef.current) return;

    // Manually sync initial state to avoid a flash of 0% if mounted mid-process
    const initialProgress = useStore.getState().progress;
    progressRef.current.style.width = `${initialProgress}%`;
    progressRef.current.setAttribute('aria-valuenow', initialProgress);

    // Subscribe directly to the store for O(1) DOM updates bypassing React
    const unsubscribe = useStore.subscribe((state) => {
      if (progressRef.current) {
        progressRef.current.style.width = `${state.progress}%`;
        progressRef.current.setAttribute('aria-valuenow', state.progress);
      }
    });

    return unsubscribe;
  }, [processing]);

  if (!processing) return null;

  return (
    <div className="fixed bottom-0 left-0 w-full h-1.5 bg-zinc-900 z-50">
      <div
        ref={progressRef}
        role="progressbar"
        aria-valuemin="0"
        aria-valuemax="100"
        className="h-full bg-blue-600 transition-all duration-500 ease-out shadow-[0_0_10px_rgba(37,99,235,0.5)]"
        // style attribute is managed by the ref after initial mount
      />
    </div>
  );
};

export default ProgressBar;
