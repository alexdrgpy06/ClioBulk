...
const ProgressBar = () => {
  // Only subscribe to 'processing' to avoid re-renders on 'progress' updates
  ...
  useEffect(() => {
    ...
    // Manually sync the initial DOM state
    if (barRef.current) {
      barRef.current.style.width = `${useStore.getState().progress}%`;
    }

    // Subscribe to the entire state and manually compare progress
