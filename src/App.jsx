            ...
            <div className="space-y-8">
              <section>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {PRESETS.map(preset => {
                    const isActive = preset.options.brightness === processingOptions.brightness &&
                                     preset.options.contrast === processingOptions.contrast &&
                                     preset.options.saturation === processingOptions.saturation &&
                                     preset.options.adaptive_threshold === processingOptions.adaptive_threshold &&
                                     preset.options.denoise === processingOptions.denoise;
                    return (
                      <button
                        key={preset.name}
                        onClick={() => setProcessingOptions(preset.options)}
                        aria-pressed={isActive}
                        className={`px-3 py-2 border rounded-lg text-xs font-medium transition-all text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${isActive ? 'bg-blue-500/10 border-blue-500 text-blue-400' : 'bg-zinc-900 border-zinc-800 hover:bg-zinc-800 hover:border-zinc-600'}`}
                      >
                        {preset.name}
                      </button>
                    );
