...
function App() {
  ...
  // O(1) lookup map for fast file ID resolution during high-frequency progress events
      ...
      const setupListener = async () => {
        const unlisten = await listen('process-progress', (event) => {
          ...
          if (stage === 'completed' || stage === 'failed') {
            ...
            // High-frequency events from Tauri can block the main thread if lookups are slow.
  ...
  const startProcessing = useCallback(async () => {
    ...
    // Populate O(1) file lookup map before processing starts
