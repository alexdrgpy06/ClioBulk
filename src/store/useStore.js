...
export const useStore = create((set) => ({
  files: [],
  fileIdsByPath: {}, // O(1) lookup mapping: path/name -> id
  processing: false,
  ...
  addFiles: (newFiles) => set((state) => {
    ...
    processedNewFiles.forEach(f => {
      const pathKey = f.path || (f.file ? f.file.name : null);
      if (pathKey) newFileIdsByPath[pathKey] = f.id;
    });
  ...
  removeFile: (id) => set((state) => {
    ...
    if (fileToRemove) {
      const pathKey = fileToRemove.path || (fileToRemove.file ? fileToRemove.file.name : null);
      if (pathKey) delete newFileIdsByPath[pathKey];
    }
