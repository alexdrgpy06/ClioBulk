...
export const useStore = create((set) => ({
  files: [],
  fileIdsByPath: {}, // O(1) lookup dictionary mapping path to file ID
  processing: false,
  ...
  addFiles: (newFiles) => set((state) => {
    const newlyAddedFiles = newFiles.map(f => ({
      file: f.file || null,
      path: f.path || null,
    ...
    for (const f of newlyAddedFiles) {
    ...
    return {
      files: [...state.files, ...newlyAddedFiles],
