...
export const useStore = create((set) => ({
  files: [],
  fileIdsByPath: {}, // Path/Name -> id lookup for O(1) retrieval
  processing: false,
  ...
  removeFile: (id) => set((state) => {
    ...
    let newFileIdsByPath = state.fileIdsByPath;

    if (fileToRemove) {
      const pathKey = fileToRemove.path || fileToRemove.file?.name;
      if (pathKey && newFileIdsByPath[pathKey] === id) {
        newFileIdsByPath = { ...state.fileIdsByPath };
        delete newFileIdsByPath[pathKey];
      }
    }
