...
## 2024-07-28 - [Screen Reader Support for Settings Sliders]
**Learning:** React range inputs acting as settings sliders require explicit `id` and `htmlFor` label associations. Even if a label is visually close, without semantic connection, screen readers simply announce "slider" without context, making settings completely unusable for visually impaired users.
**Action:** Always link `<label>` and `<input type="range">` elements using matching `htmlFor` and `id` attributes when building custom control panels to ensure the setting name is announced.
