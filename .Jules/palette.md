...
## 2024-05-26 - [Form Control Accessibility]
**Learning:** Standard HTML5 controls like `<input type="range">` and dynamically rendered `<button>` elements do not inherit global focus styles in this project. Additionally, range inputs need explicit `aria-valuetext` when the displayed value is mathematically derived (like percentage or multiplier) rather than the raw decimal value.
**Action:** Explicitly apply Tailwind `focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none` classes, and provide `aria-valuetext` with proper `<label>` `htmlFor`/`id` bindings to ensure accessibility for settings controls.
