...
## 2025-02-12 - [Missing Focus States on Standard Inputs]
**Learning:** Standard `<input type="range">` elements do not always inherit the application's global focus styles, leaving keyboard users without a clear indication of which slider is active. The same applies to buttons that are part of dynamic lists or panels (like preset options).
**Action:** Always verify that every interactive element, including standard HTML form controls and dynamically rendered list items, explicitly implements the design system's `focus-visible` classes (e.g., `focus-visible:ring-2 focus-visible:ring-blue-500`) to ensure consistent keyboard accessibility across the entire UI.
