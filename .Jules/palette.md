...
## 2024-10-24 - [Accessible Range Inputs]
**Learning:** Range inputs for settings (like Brightness/Contrast) that lack explicitly associated labels via `htmlFor` and `id` are not properly announced by screen readers, making them difficult for visually impaired users to interact with.
**Action:** Always use explicitly matching `htmlFor` on the `<label>` and `id` on the `<input type="range">` elements.
