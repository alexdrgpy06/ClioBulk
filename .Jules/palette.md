...
## 2024-05-12 - Missing Focus Styles on Standard Controls
**Learning:** In this project's UI, standard HTML5 controls like `<input type="range">` and dynamically rendered `<button>` elements do not automatically inherit global focus styles.
**Action:** Explicitly apply Tailwind `focus-visible` utility classes (e.g., `focus-visible:ring-2`) and provide `aria-valuetext` along with proper `<label>` `htmlFor`/`id` bindings to ensure full keyboard and screen reader accessibility.
