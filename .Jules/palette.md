...
## 2024-05-13 - Explicit Accessibility for Native Controls
**Learning:** Standard HTML5 controls like `<input type="range">` and dynamically rendered `<button>` elements do not automatically inherit global focus styles in this project's UI. Additionally, range inputs require `aria-valuetext` when the visual value presentation (like percentages) differs from the raw input `value`.
**Action:** Always explicitly apply Tailwind `focus-visible` utility classes (e.g., `focus-visible:ring-2`) and provide `aria-valuetext` along with proper `<label>` `htmlFor`/`id` bindings to ensure full keyboard and screen reader accessibility.
