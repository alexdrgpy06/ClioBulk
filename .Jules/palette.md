...
## 2024-05-24 - Explicit Label Associations for Range Sliders
**Learning:** Range inputs for adjustments were missing explicit label associations (`htmlFor` matching `id`), preventing screen readers from correctly announcing the setting name when the slider is focused.
**Action:** Always explicitly associate `<label>` and `<input>` elements using matching `htmlFor` and `id` attributes for custom settings sliders and range inputs to ensure full keyboard and screen reader accessibility.
