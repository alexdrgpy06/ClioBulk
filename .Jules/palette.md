...
## 2024-07-28 - [Accessible Form Controls and Global Focus]
**Learning:** Standard HTML5 controls like `<input type="range">` and dynamically rendered `<button>` elements do not inherit global focus styles in this project. Additionally, dynamically rendered sliders are inaccessible without explicitly binding `<label>` elements via `htmlFor` and providing `aria-valuetext`.
**Action:** Always explicitly apply Tailwind `focus-visible` utility classes (e.g., `focus-visible:ring-2`) and provide `aria-valuetext` with proper `<label>` `htmlFor`/`id` bindings to ensure accessibility for dynamically rendered controls.
