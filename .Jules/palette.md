...
## 2024-07-28 - [Form Controls Accessibility]
**Learning:** Standard HTML5 controls like `<input type="range">` and dynamically rendered `<button>` elements do not automatically inherit global focus styles in this project. Additionally, range inputs need explicitly associated `<label>` tags and `aria-valuetext` for screen readers to properly read values and changes.
**Action:** Explicitly apply Tailwind `focus-visible` utility classes (e.g., `focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none`) to inputs and dynamic buttons, and provide `aria-valuetext` with proper `<label>` `htmlFor`/`id` bindings.
