...
## 2026-05-01 - [Keyboard Accessibility for Settings Panel]
**Learning:** Standard HTML5 controls like `<input type="range">` and dynamically rendered `<button>` elements (like the presets) do not automatically inherit global focus styles in this project's UI. Also, sliders lack screen-reader friendly value readouts without aria-valuetext.
**Action:** Explicitly apply Tailwind `focus-visible` utility classes (e.g., `focus-visible:ring-2`) to interactive elements. Provide `aria-valuetext` along with proper `<label>` `htmlFor`/`id` bindings for sliders to ensure full keyboard and screen reader accessibility.
