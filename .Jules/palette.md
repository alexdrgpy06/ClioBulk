...
## 2024-10-24 - [Missing Focus Styles on Native Controls]
**Learning:** In this project's UI, standard HTML5 controls like `<input type="range">` and dynamically rendered preset `<button>` elements do not automatically inherit global focus styles, making them inaccessible via keyboard navigation.
**Action:** Always explicitly apply Tailwind `focus-visible` utility classes (e.g., `focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none`) and provide `aria-valuetext` along with proper `<label>` `htmlFor`/`id` bindings for full keyboard and screen reader accessibility.
