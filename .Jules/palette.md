...
## 2024-10-27 - [Focus States and Labels for Range Inputs]
**Learning:** Standard HTML5 `<input type="range">` elements and dynamically rendered list buttons do not automatically inherit global focus styles. They require explicit Tailwind focus-visible classes (`focus-visible:ring-2`) to ensure keyboard accessibility. Range inputs also need `aria-valuetext` and `<label>` linkage via `id` and `htmlFor`.
**Action:** Always add explicit focus-visible classes, `aria-valuetext`, and `<label>` linkages when using native range inputs and custom buttons to maintain full accessibility.
