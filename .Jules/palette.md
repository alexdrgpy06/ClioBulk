...
## 2024-07-25 - [Range Input Focus & Accessibility]
**Learning:** Standard HTML5 controls like `<input type="range">` in this project do not inherit global focus styles, making them invisible to keyboard navigation, and lack default screen reader associations.
**Action:** Always explicitly apply Tailwind `focus-visible` utility classes (e.g., `focus-visible:ring-2`) and provide `aria-valuetext` with proper `<label>` `htmlFor`/`id` bindings for custom range inputs.
