...
## 2024-06-26 - [Form Input Accessibility]
**Learning:** Custom settings sliders or range inputs in React often lack proper label associations, causing screen readers to fail in announcing the setting name to users.
**Action:** Explicitly associate the `<label>` and `<input>` elements using matching `htmlFor` and `id` attributes whenever building custom forms or settings.
