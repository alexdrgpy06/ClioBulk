...
## 2024-08-14 - [Range Input Accessibility]
**Learning:** In the React settings panel, `<label>` elements for range sliders were visually associated but lacked programmatic `htmlFor` bindings to the inputs' `id` attributes, meaning screen readers would announce unlabelled sliders.
**Action:** Always explicitly associate `<label>` elements with their corresponding `<input>` fields using matching `htmlFor` and `id` attributes, even if they are grouped visually inside flex containers.
