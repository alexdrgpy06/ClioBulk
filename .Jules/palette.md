...
## 2024-10-25 - [Missing Focus Styles on HTML5 Controls]
**Learning:** Standard HTML5 controls like `<input type="range">` and dynamically rendered `<button>` elements in this project do not inherit global focus styles, making them inaccessible to keyboard users navigating through settings or presets.
**Action:** Explicitly apply Tailwind `focus-visible` utility classes (e.g., `focus-visible:ring-2`) to these interactive elements and provide `aria-valuetext` along with proper `<label>` `htmlFor`/`id` bindings to ensure full keyboard and screen reader accessibility.
