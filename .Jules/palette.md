...
## 2024-06-25 - [Preset Active States in Adjustment Panels]
**Learning:** Users can lose track of active configurations when predefined preset buttons act as fire-and-forget actions without maintaining an active state representation.
**Action:** When implementing predefined configuration presets, always compute an active state (via `aria-pressed` and visual styling) by checking the current configuration options.
