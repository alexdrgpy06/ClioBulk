...
## 2026-08-23 - [Explicit Label Associations for Range Inputs]
**Learning:** Custom settings sliders using range inputs in React must have explicitly associated `<label>` and `<input>` elements using `htmlFor` and `id` attributes to ensure screen readers correctly announce the setting name.
**Action:** Always verify that custom range inputs or sliders are properly associated with their labels.
