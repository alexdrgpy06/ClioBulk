...
## 2026-07-08 - Added accessible ID and htmlFor to custom settings sliders
**Learning:** When building custom settings sliders or range inputs in React, explicitly associate the `<label>` and `<input>` elements using matching `htmlFor` and `id` attributes to ensure screen readers correctly announce the setting name.
**Action:** Always check that input elements have associated labels with matching ID and htmlFor attributes.
