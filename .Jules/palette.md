...
## 2024-07-28 - [Accessible Native Controls and Dynamic Lists]
**Learning:** Standard HTML5 controls like range inputs (`<input type="range">`) and dynamically rendered list buttons do not automatically inherit global focus styles in Tailwind. Without explicit `focus-visible` classes, keyboard users cannot see their focus state. Furthermore, range inputs require `aria-valuetext` and explicit `<label>` linkage via `id` and `htmlFor` to be meaningful to screen readers.
**Action:** Always add explicit `focus-visible:ring-2` (and appropriate outline/offset) classes to all interactive elements, including native range inputs and dynamically mapped buttons. Always explicitly link range inputs to their labels and provide readable `aria-valuetext` values.
