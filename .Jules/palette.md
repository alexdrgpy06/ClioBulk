...
## 2024-05-25 - Range Input Accessibility
**Learning:** Native `<input type="range">` elements do not inherit global focus styles in this project, and often lack explicit `<label>` bindings, resulting in poor keyboard accessibility and screen reader experience.
**Action:** Explicitly apply Tailwind `focus-visible:ring-2` utility classes and provide `aria-valuetext` with proper `<label>` `htmlFor`/`id` bindings for all range inputs to ensure accessibility.
