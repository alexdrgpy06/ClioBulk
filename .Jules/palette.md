...
## 2024-05-24 - Missing Focus Styles on Native Controls
**Learning:** Standard HTML5 controls like `<input type="range">` and dynamically rendered `<button>` elements do not inherit global focus styles in this project, negatively impacting keyboard accessibility.
**Action:** Explicitly apply Tailwind `focus-visible` utility classes (e.g., `focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none`) and provide `aria-valuetext` with proper `<label>` `htmlFor`/`id` bindings to ensure accessibility.
