...
## 2024-06-29 - [Accessible Range Inputs]
**Learning:** When building custom settings sliders or range inputs, the screen reader does not automatically associate text nearby as the label. `<label>` tags without `htmlFor` fail to provide programmatic context to screen readers, a common issue in our custom sidebars.
**Action:** Always explicitly associate the `<label>` and `<input>` elements using matching `htmlFor` and `id` attributes to ensure screen readers correctly announce the setting name when the user focuses on the slider.
