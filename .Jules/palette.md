...
## 2024-07-28 - [Accessible Custom Progress Bars]
**Learning:** Custom progress bars implemented with generic `div` elements lack semantics and state visibility for screen readers, meaning users are unaware of background processing states.
**Action:** Always apply `role="progressbar"`, `aria-label`, `aria-valuenow`, `aria-valuemin`, and `aria-valuemax` attributes to custom progress indicators to expose their purpose and current value to assistive technologies.
