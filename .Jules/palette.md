...
## 2024-07-16 - [Standard Control Keyboard Focus]
**Learning:** Standard HTML5 controls like range inputs (`<input type="range">`) and dynamically rendered list buttons do not automatically inherit global focus styles in Tailwind unless explicitly defined. Range inputs also need `aria-valuetext` and `<label>` linkage via `id` and `htmlFor`.
**Action:** Ensure explicit Tailwind `focus-visible` classes (e.g., `focus-visible:ring-2 focus-visible:ring-offset-2`) are added to native input elements and dynamically mapped interactive elements, alongside proper ARIA semantic associations.
