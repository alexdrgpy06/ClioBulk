...
## 2026-08-17 - [Accessible Range Inputs]
**Learning:** Range inputs for settings like brightness were missing proper label associations (htmlFor/id), causing screen readers to announce them as generic sliders without context.
**Action:** Always explicitly associate custom settings sliders or range inputs with their corresponding <label> elements using matching htmlFor and id attributes to ensure screen readers correctly announce the setting name.
