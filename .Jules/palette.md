...
## 2024-07-20 - [Range Inputs and Dynamic Elements Accessibility]
**Learning:** Standard HTML5 controls like `<input type="range">` and dynamically rendered `<button>` elements do not inherit global focus styles in this project, making them inaccessible for keyboard users navigating the "Adjustments" pane.
**Action:** Explicitly apply Tailwind `focus-visible` utility classes (e.g., `focus-visible:ring-2`) and provide `aria-valuetext` with proper `<label>` `htmlFor`/`id` bindings to ensure accessibility on all native and dynamic inputs.
