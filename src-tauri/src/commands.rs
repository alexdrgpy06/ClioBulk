...
pub fn process_image_inner<R: Runtime>(
    ...
    // Security: Prevent arbitrary file writes by whitelisting output extensions.
    let out_path_lc = out_path.to_lowercase();
    if !out_path_lc.ends_with(".jpg") && !out_path_lc.ends_with(".jpeg") && !out_path_lc.ends_with(".png") && !out_path_lc.ends_with(".webp") {
        let err_msg = "Security Error: Arbitrary file write prevented. Only .jpg, .jpeg, .png, and .webp extensions are allowed.".to_string();
        error!("{}", err_msg);
        emit("failed", false, Some(err_msg.clone()));
        return ProcessResult {
            success: false,
