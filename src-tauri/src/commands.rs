...
pub fn process_image_inner<R: Runtime>(
    ...
    // Security Enhancement: Restrict output file extensions to prevent arbitrary file write vulnerabilities
    ...
    if !allowed_extensions.iter().any(|ext| out_path_lc.ends_with(*ext)) {
        let err_msg = format!("Invalid file extension for output path: {}", out_path);
        error!("{}", err_msg);
        emit("failed", false, Some(err_msg.clone()));
        return ProcessResult {
            success: false,
