...
pub fn process_image_inner<R: Runtime>(
    ...
    // Explicitly validate output extension to prevent arbitrary file writes
    let out_path_path = std::path::Path::new(&out_path);
    let valid_exts = ["jpg", "jpeg", "png", "webp"];
    let ext = out_path_path.extension()
        .and_then(|e| e.to_str())
        .map(|e| e.to_lowercase())
        .unwrap_or_default();

    if !valid_exts.contains(&ext.as_str()) {
        let err_msg = format!("Invalid output file extension: {}", ext);
        error!("{}", err_msg);
        emit("failed", false, Some(err_msg.clone()));
        return ProcessResult {
            success: false,
