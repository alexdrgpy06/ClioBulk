...
pub fn process_image_inner<R: Runtime>(
    ...
    // Security: Whitelist allowed output file extensions to prevent arbitrary file writes
    let out_path_lc = out_path.to_lowercase();
    let allowed_extensions = [".jpg", ".jpeg", ".png", ".webp"];
    if !allowed_extensions.into_iter().any(|ext| out_path_lc.ends_with(ext)) {
        let err_msg = format!("Invalid output file extension. Allowed extensions are: {:?}", allowed_extensions);
        error!("{}", err_msg);
        emit("failed", false, Some(err_msg.clone()));
        return ProcessResult {
            success: false,
