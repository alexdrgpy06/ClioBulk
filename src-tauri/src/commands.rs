...
pub fn process_image_inner<R: Runtime>(
    ...
    if !out_path_lc.ends_with(".jpg") && !out_path_lc.ends_with(".jpeg") && !out_path_lc.ends_with(".png") {
        let err_msg = format!("Invalid output file extension. Only .jpg, .jpeg, and .png are allowed: {}", out_path);
        error!("{}", err_msg);
        emit("failed", false, Some(err_msg.clone()));
        return ProcessResult {
            success: false,
