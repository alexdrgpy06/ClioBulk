...
pub fn process_image_inner<R: Runtime>(
    ...
    // Security check: Only allow writing specific image formats to prevent arbitrary file writes
    ...
    if !allowed_exts.iter().any(|ext| out_path_lc.ends_with(*ext)) {
        let err_msg = format!("Security error: Invalid output file extension. Only {:?} are allowed", allowed_exts);
        error!("{}", err_msg);
        emit("failed", false, Some(err_msg.clone()));
        return ProcessResult {
            success: false,
