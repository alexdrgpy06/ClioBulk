...
pub async fn process_bulk(app: AppHandle, files: Vec<(String, String)>, options: ProcessOptions) -> Result<(), String> {
    ...
    for (i, (in_p, out_p)) in files.into_iter().enumerate() {
        ...
        let handle = tokio::spawn(async move {
            ...
            if let Err(e) = res {
                error!("Task execution failed for {}: {}", in_p_err, e);
                let _ = app_err.emit("process-progress", ProgressPayload {
                    path: in_p_err,
                    success: false,
                    error: Some(format!("Internal processing error: {}", e)),
                    progress,
